    ...
    <form action="" method="post">
        <table>
            ...
            <tr id="header"><td><input type="checkbox" id="all" onclick="checkall()"/></td>
                <td>产品id</td><td>销售总额</td><td>销售总量</td></tr>
            <c:forEach var="userbuy" items="${listuser}" begin="0" end="${listuser.size()}">
                <tr>
                    <td class="chb"><input type="checkbox" name="chb" value="${goods.getId()}"></td>
                    <td>${userbuy.getName()}</td><td>${userbuy.getProfit()}</td>
                    <td>${userbuy.getNumber()}</td>
                </tr>
