...
<head>
    ...
    <style type="text/css">
        *{margin: 0px;padding: 0px;}
        td{border: 1px solid pink;padding: 2px 0px;}
        table{border-collapse: collapse; margin: 0px auto;width:1024px;}
        table img{width: 100px;height: 100px;}
        #table1 td{border: 0px;}
        #header{background-color: pink;color: white;font-weight: bold;text-align: center;line-height: 28px;}
...
<body>
<div>
    <form action="GoodServlets?opr=fuzzy&from=admin" method="post">
        <table id="table1">
    ...
    <form action="GoodsServlet?opr=mdel" method="post">
        <table>
            ...
            <tr id="header"><td><input type="checkbox" id="all" onclick="checkall()"/></td>
                <td>商品编号</td><td>商品名称</td><td>商品价格</td><td>商品库存</td><td>图片一</td><td>图片二</td><td>图片三</td><td>操作</td></tr>
            <c:forEach var="goods" items="${listgoods}" begin="0" end="${listgoods.size()}">
                <tr>
                    ...
                    <td><img src="img/${goods.getImg3()}"/></td>
                    <td style="text-align: center;"><a href="updateGoods.jsp?id=${goods.getId()}&name=${goods.getName()}&price=${goods.getPrice()}&stock=${goods.getStock()}&img1=${goods.getImg1()}&img2=${goods.getImg2()}&img3=${goods.getImg3()}">修改</a><br><br><a href="GoodsServlet?opr=del&id=${goods.getId()}" class="del">删除</a></td>
                </tr>
